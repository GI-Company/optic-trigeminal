...
bool ClinicalAnalyzer::is_warning_threshold(int value, std::string vital_type) {
    if (vital_type == "hr") {
        return ((value > hr_thresholds.warning_low && value < hr_thresholds.normal_low) ||
                (value > hr_thresholds.normal_high && value < hr_thresholds.warning_high));
    } else if (vital_type == "spo2") {
        return (value > spo2_thresholds.warning_low && value < spo2_thresholds.normal_low);
    } else if (vital_type == "bp_sys") {
        return ((value > bp_sys_thresholds.warning_low && value < bp_sys_thresholds.normal_low) ||
                (value > bp_sys_thresholds.normal_high && value < bp_sys_thresholds.warning_high));
    } else if (vital_type == "rr") {
        return ((value > rr_thresholds.warning_low && value < rr_thresholds.normal_low) ||
                (value > rr_thresholds.normal_high && value < rr_thresholds.warning_high));
    }
    return false;
}
