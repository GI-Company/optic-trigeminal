...
void ScenarioRuntime::mutate_vitals_baseline() {
    ...
    current_vitals_.hr = std::max(40, std::min(160, current_vitals_.hr));
...
float ScenarioRuntime::evaluate_condition(const std::string& expr) {
    ...
    if (expr.find("no_action_within_20_min") != std::string::npos) {
        return (elapsed_time_sec_ > 1200 && active_actions_.empty()) ? 1.0f : 0.0f;
    }
